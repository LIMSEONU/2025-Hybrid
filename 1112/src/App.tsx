...
function App() {
  ...
  useEffect(() => {
      ...
      if (window.navigator && window.navigator.geolocation) {
        window.navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            ...
            console.log('⚠️ 기본 위치 사용:', defaultLocation);
    ...
    // Cordova 환경인지 확인
    if (window.cordova) {
      console.log('🔧 Cordova 환경 감지 - deviceready 대기 중...');
    ...
    return () => {
      if (window.cordova) {
        document.removeEventListener('deviceready', getLocation);
      }
    };
  }, []);
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 to-orange-50">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-500 mt-2">GPS 신호를 찾고 있습니다...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 flex h-screen">
        ...
        {/* 메인 컨텐츠 */}
        <main className="flex-1 overflow-y-auto pt-14 lg:pt-0 h-full">
          {currentPage === "home" && (
            <HomePage
